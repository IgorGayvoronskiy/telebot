...
def run_style_transfer(cnnf, normalization_mean, normalization_std,
                       content_imgf, style_imgf, input_imgf, num_steps=400,
...
token = "6089321080:AAG8m-pdqU4XNDCL1tLbsXcqvq4ihwf5kJQ"
bot = telebot.TeleBot(token, parse_mode=None)


@bot.message_handler(commands=['help'])
def send_tusk(message):
    bot.send_message(message.chat.id, "Я могу передать стиль одной фотографии - другой. Чтобы начать работу, пришлите фотографию, на которую хотите перенести стиль")
...
def handle_docs_photo(message):
    ...
    if not path.exists(src1):
        ...
        with open(src2, 'wb') as new_file:
            new_file.write(downloaded_file)
        bot.reply_to(message, "Фотография получена. Результат будет через несколько минут, ждите...")
        res = join_photo()
...
def send_welkom(message):
    bot.send_message(message.chat.id, "Здравствуйте! Напишите команду /help, чтобы узнать, что я могу")
